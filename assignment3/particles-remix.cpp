...
// based on original code by Stejara Dinulescu 
...
struct AlloApp : App {
  ...
  // simplest way to not get NANs, keep timestep small
  Parameter gravConst{"/gravConst", "", 0.002, "", 0, 1};
  Parameter dragFactor{"/dragFactor", "", 0.5, "", 0, 0.99};
  Parameter maxAccel{"/maxAccel", "", 30, "", 0, 100};
  Parameter symmetry{"/symmetry", "", 1, "", 0, 1};
  Parameter springLength{"SPRING LENGTH", "", 5, "", 0, 5};
  Parameter k{"SPRING CONSTANT K", "", 1, "", 0, 2};
  // add GUI params here
  ControlGUI gui;
  ...
  void reset() { // empty all containers
    ...
    for (int r = 0; r < partNum; r++) { // create 1000 points, put it into mesh
      ...
      if (r == 0) {  //
        ...
        mesh.vertex(0);
        mesh.color(1, 1, 1);
        velocity.push_back(0); // start with some small velocity
      } else if (r > 0 && r <= 9) {
      ...
      acceleration.push_back(0); // start with no acceleration
    }
  }

  void onCreate() override {
    // add more GUI here
    gui << pointSize << timeStep << dragFactor << maxAccel << symmetry
        << springLength << k; // stream operator
    gui.init();
  ...
  void onAnimate(double dt) override {
    ...
    for (int i = 0; i < partNum;
      ...
      for (int j = 1 + i; j < partNum; j++) {
        Vec3f distance(mesh.vertices()[j] -
                       mesh.vertices()[i]); // calculate distances between
                                            // particles -> b-a = c
        Vec3f gravityVal = G * mass[i] * mass[j] * distance.normalized() /
                           pow(distance.mag(), 2); // F = G * m1 * m2 / r^2
        // multiply by r hat -> only direction, normalized magnitude
        Vec3f springVal =
            distance.normalized() * ((distance.mag() - springLength) * k);

        acceleration[i] += (gravityVal / mass[i]) + springVal;
        acceleration[j] -= (gravityVal * symmetry / mass[j]) + springVal;
      }
    }
  ...
  bool onKeyDown(const Keyboard &k) override {
    ...
    if (k.key() == '1') {
      ...
      for (int i = 0; i < partNum; i++) {
        acceleration[i] =
            rv(1) /
            mass[i]; // rv gives a force -> divide by mass to get acceleration
